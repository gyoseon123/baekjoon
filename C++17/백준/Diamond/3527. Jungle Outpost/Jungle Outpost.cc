...
bool check(int itv, const vector<Point> &points){
    vector<Line> new_lines;
    for (int i = 0; i < sz(points); i++){
        Point p1 = points[i];
        Point p2 = points[(i+itv)%sz(points)];
        new_lines.push_back(Line(p1, p2));
    }
    if (new_lines.size() >= 3){
        swap(new_lines[0], new_lines[1]);
        swap(new_lines[0], new_lines[2]);
    }
