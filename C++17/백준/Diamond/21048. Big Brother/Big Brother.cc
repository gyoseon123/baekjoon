...
vector<Point> convex_hull(vector<Point> &points){ // ch를 ccw순으로 반환
    ...
    vector<Point> ret;
    for (int i = 0; i < sz(stk2); i++) ret.push_back(stk2[i]);
    for (int i = sz(stk1)-2; i >= 1; i--) ret.push_back(stk1[i]);
    return ret;
}
...
Point find_point(Point p1, Point p2, Point p3){ // 로테이팅 캘리퍼스 보조함수
    return Point(p3.x - p2.x + p1.x, p3.y - p2.y + p1.y);
}
ld find_max_dist(vector<Point> &points){ // ch을 입력하면 최대거리를 반환 ch는 ccw순일것
    int n = points.size();
    int r = 0;
    ld ret = 0;
...
bool is_hide(Line a, Line b, Line c){ // HPI 보조함수
    Point p;
...
vector<Point> HPI(vector<Line> lines){ // HPI를 ccw순으로 반환
