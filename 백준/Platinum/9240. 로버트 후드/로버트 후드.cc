...
int is_in(vector<Point> &convex_hull, Point p){ // 내부 점 판정
    int cnt = 0;
...
int is_cross(Point p1, Point p2, Point p3, Point p4){ // 선분교차판정
...
ld find_max_dist(vector<Point> &points){ // ch을 입력하면 최대거리를 반환
    int n = points.size();
    int r = 0;
    ld ret = 0;

    for (int i = 0; i < n; i++){
        while (r < 2*n && cross(points[i], points[(i+1)%n], find_point(points[(i+1)%n], points[r%n], points[(r+1)%n])) <= 0){
